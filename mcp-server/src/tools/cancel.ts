  ...
  if (!specName) {
    return {
      content: [
        {
          type: "text",
          text: "Error: No spec specified and no current spec set.\n\nUse ralph_switch to select a spec or provide spec_name parameter.",
        },
      ],
    };
  }
  ...
  if (!fileManager.specExists(specName)) {
    return {
      content: [
        {
          type: "text",
          text: `Error: Spec "${specName}" not found.`,
        },
      ],
    };
  }
